...
object SparkProceraCompute {
  ...
  def doAnonymize(rdd: RDD[Array[String]]) = {
    try {
      rdd.collect.foreach(record => println(record(0) + ", " + record(1)))
    } catch {
  ...
  def run(kafkaParams: Map[String, String], topics: Set[String], interval: Int) {
    ...
    csv.foreachRDD(rdd => doAnonymize(rdd))
    ssc.start()
  ...
  def main(args: Array[String]) {
    ...
    run(kafkaParams, topics, interval.toInt)
